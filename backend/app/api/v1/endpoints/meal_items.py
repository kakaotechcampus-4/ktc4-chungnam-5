...
def update_meal_items(
    ...
    입력한 숫자·단위는 `raw_ai_result` 에 남긴다 — `source=USER` 는 그 자리가 원래
    사용자 입력이라 통째로 갱신하고(`POST` 와 같은 모양), `source=MODEL` 은 AI 원본을
    덮을 수 없어 `userInput` 키 아래에 따로 둔다. 이게 없으면 "계란 2개 → 3개" 같은
