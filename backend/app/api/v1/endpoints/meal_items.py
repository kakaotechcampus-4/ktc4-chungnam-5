...
def delete_meal_item(
    ...
    `meal_items` 에는 `deleted_at` 이 없다. 행을 지우면 `user_corrections` 도 FK 의
    `ondelete=CASCADE` 로 따라 사라지고, `original_food_name` · `estimated_*` 에
    ...
    🔗 **`POST /meals/{mealId}/confirm` 이 항목 0 개 식사를 받게 된다.** 그 상태로
    Q/Q/S 를 채점하면 0 kcal 짜리 점수가 나가므로, confirm 은 이 경우를 따로
    정해야 한다(거부할지, `FAILED` 로 보낼지). confirm 작업에 이 문단을 함께
    전달할 것 — `add_meal_item` 독스트링의 의존성 항목과 같은 자리다.
    """
    try:
