...
Figma 에 해당 상태 화면이 **없어서** 규칙을 정해 만든 것이다.
구현은 `lib/screens/shared_meal_widgets.dart` 에 모여 있고, 새 화면도 이 위젯을 쓴다.
직접 만들지 말고 여기 것을 가져다 쓴다 — 화면마다 다른 실패 화면이 생기면 그때는 못 되돌린다.

### 원칙

- **새 색 토큰을 만들지 않는다.** 스켈레톤은 `surfaceMuted`, 안내·실패 블록은 `primaryTint` 배경 + `textPrimary` 텍스트.
- **에러 전용 색을 만들지 않는다.** 팔레트에 에러색이 없고(§2), 코랄을 쓰면 "강조색은 화면당 한 군데"와 부딪힌다.
  Q·Q·S 세 색은 의미가 고정이라 쓸 수 없다. 그래서 **색이 아니라 블록 형태로** 실패를 구분한다.
- **애니메이션 없는 정적 스켈레톤.** shimmer 는 패키지 의존성이 필요하다.
- **스켈레톤은 완료 상태와 높이가 같아야 한다.** 다르면 로딩에서 완료로 넘어갈 때 화면이 튄다.
- **API 가 둘 이상인 화면은 블록마다 상태를 따로 들고 있는다.** 한쪽 실패가 다른 쪽을 같이 죽이지 않는다.
- **409 `PROFILE_REQUIRED` · `STAGE_NOT_SET` 은 실패가 아니라 이동이다.** 재시도 블록을 띄우지 않는다.
- 카피는 §7 의 톤을 따른다. 평서형 존댓말, 사용자를 탓하지 않는다.

### 위젯

| Dart | 용도 |
|---|---|
| `LoadState` | `loading` · `ready` · `failed`. 화면 또는 블록 단위로 들고 있는다 |
| `SkeletonBox` | 스켈레톤 한 조각 |
| `MealCardSkeleton` | `MealCard` 로딩 자리. 높이·패딩이 `MealCard` 와 같다 |
| `RetryBlock` | 실패 + `다시 시도` 보조 버튼 |
| `EmptyBlock` | 보여 줄 게 없는 상태. 실패가 아니므로 버튼을 두지 않는다 |
| `NoticeBlock` | `safetyStatus: BLOCKED` 상담 안내 |

### 기본 카피

| 상태 | 문구 |
|---|---|
| 실패 | `잠시 후 다시 시도해 주세요` + 버튼 `다시 시도` |
| 빈 상태 | `아직 기록된 식사가 없어요` |
| 안전 차단 | `지금은 피드백을 드리기 어려워요. 담당 의사와 상담해 주세요.` |

### 화면별 적용 (1 · 2 · 9)

| 화면 | 로딩 | 실패 | 그 밖 |
|---|---|---|---|
| 2 홈 | 날짜는 즉시. 투약 카드·식사 목록은 스켈레톤, **게이지는 실루엣을 남기고 채움만 0%** | 단일 API 라 화면 전체가 `RetryBlock` | 0끼면 게이지 0% + `EmptyBlock`. `BLOCKED` 면 팁 문장 자리만 `NoticeBlock`, 나머지는 정상 |
| 1 투약 정보 | 프리필은 스켈레톤 없이 기본값으로 즉시 렌더 | 버튼 위 인라인 `RetryBlock`, 입력값 유지 | 저장 중에는 버튼 비활성 (`다음` → `저장 중`) |
| 9 기록 | 달력은 그리드를 남기고 **점만 비운다**. 목록은 `MealCardSkeleton` 2장 | 달력·목록 각각 따로. 한쪽만 재시도 | 그 날 0끼면 `EmptyBlock` + 기록 버튼 |

> 홈의 팁 문장처럼 **나중에 도착하는 문구**는 실패해도 그 줄만 빠지고 나머지는 정상이어야 한다(§7 규칙 3).
