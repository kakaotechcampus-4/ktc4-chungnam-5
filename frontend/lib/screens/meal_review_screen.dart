...
// ── 데이터 모델 ────────────────────────────────────────────
// API 명세서(GET /meals/{mealId}) 응답 구조 그대로 반영. 확정된 내용.
...
// ── API 서비스 ────────────────────────────────────────────
// "무엇을 보내고 받는지"는 명세서 기준으로 확정.
// "어떻게 보내는지"(http/dio)는 팀 결정 후 채울 자리 — 지금은 더미로 동작.

class MealApiService {
  /// GET /meals/{mealId}
  Future<MealDetail> fetchMeal(String mealId) async {
    // TODO(http|dio 결정 후): 실제 GET 요청으로 교체.
    // 지금은 명세서 예시값을 그대로 돌려줘서 화면이 동작하게만 해둠.
  ...
  /// PATCH /meals/{mealId}/items — body: { items: [{ itemId, displayName, amount, unit }] }
  /// 응답: { status: "ANALYZING", isRecalculation: true }
  Future<bool> updateItem(String mealId, MealItem item) async {
    // TODO(http|dio 결정 후): 실제 PATCH 요청으로 교체.
    await Future.delayed(const Duration(milliseconds: 200));
    return true; // isRecalculation 자리
  }

  /// POST /meals/{mealId}/items — body: { displayName, amount, unit }
  Future<bool> addItem(
    String mealId,
    String displayName,
    double amount,
  ...
  Future<bool> deleteItem(String mealId, String itemId) async {
    // TODO(http|dio 결정 후): 실제 DELETE 요청으로 교체.
    await Future.delayed(const Duration(milliseconds: 200));
    return true;
  }

  /// POST /meals/{mealId}/confirm — body: { satietyAfterPct? }
...
class _MealReviewScreenState extends State<MealReviewScreen> {
  ...
  Future<void> _changeAmount(MealItem item, double delta) async {
    ...
    try {
      ...
      // TODO: isRecalculation true면 README 폴링 규칙(1.5초 간격)대로 재조회 필요.
      // http/dio 정해지고 진짜 응답 오면 이 부분 폴링 로직 추가.
    } catch (e) {
  ...
  void _addItem() {
    // TODO: 추가 UI(팝업/화면) 미정이라 아직 비워둠.
  }

  void _editItem(MealItem item) {
    // TODO: 수정 UI(팝업/화면) 미정이라 아직 비워둠.
  }
  ...
  Future<void> _confirmAndEvaluate() async {
    ...
    try {
      ...
      if (!mounted) return;
      // TODO: MealEvaluationScreen 파일 확인되면 아래 주석 해제.
      // Navigator.push(
      //   context,
      //   MaterialPageRoute(
      //     builder: (_) => MealEvaluationScreen(mealId: widget.mealId, initialData: result),
      //   ),
      // );
    } catch (e) {
